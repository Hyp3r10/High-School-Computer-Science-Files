		...
		if(this.size == 0) {
			...
			while(temp != null) {
				...
				if(distance < smallestDeltaDistance) {
					smallestDeltaNode = index;
					smallestDeltaDistance = distance;
				}
				// Reset the LinkedList to its original state
				temp.next = tempNext;
				// Increase the index of where we are currently in the LinkedList
				index++;
